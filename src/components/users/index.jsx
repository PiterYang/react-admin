...
const fixOptions = [{label:'user_type_1', key: 'user_type_1'}, {
    label: 'user_type_2', key: 'user_type_2'
}]
...
class DynamicFieldSet extends React.Component {
  ...
  addRealAudience = (index, e) => {
    if(fixOptions.map(v => v.key).includes(e)) {
        ...
        const restOpionts = keys
                ...
                .map(v => v[0].key)
        this.setState({
            selectList: fixOptions.filter(v => !restOpionts.includes(v.name))
        }) 
  ...
  formItemType = (item, index) => {
    switch(item.type) {
        ...
        case 'select':
            let selectList = Array.isArray(item.selectList) ? item.selectList : this.state.selectList
            
            return <Select onChange={(e) => this.addRealAudience(index, e)}>
                {selectList.map((options) => (
                    <Option key={options.key} value={options.key}>{options.label}</Option>
                ))}
            </Select>
  ...
  render() {
    ...
    const formItems = keys.map((k, index) => {
        return <div key={index}>{
            ...
            k.map((v,subIndex) => (
                <Form.Item
                    {...(subIndex === 0 ? formItemLayout : formItemLayoutWithOutLabel)}
